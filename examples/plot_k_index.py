...
parser = argparse.ArgumentParser(description="Plot local K index")
parser.add_argument(
...
parser.add_argument("--tries", type=int, default=3, help="Number of tries to laod a QDC")
...
parser.add_argument("--offset", default=100, type=float, help="Offset between sites in nT")
...
for n_s in args.project_site:
    ...
    assert m is not None, "Magnetometer must have form PROJECT or PROJECT/SITE"
    project = m.groups()[0].upper()
    assert project in ap.projects, "Project %s is not known" % project
    ...
    if m.groups()[2] is None:
        ...
        assert site in ap.projects[project], "Site %s/%s is not known" % (project, site)
...
for n_s in n_s_list:
    ...
    if md is not None and "MagQDC" in ap.get_site_info(project, site, "data_types"):
        ...
        qdc_info = ap.magdata.load_qdc(project, site, dt64.mean(st, et), tries=args.tries, full_output=True)
        if qdc_info:
            qdc = qdc_info["magqdc"]
