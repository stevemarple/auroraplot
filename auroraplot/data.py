...
def generic_load_converter(
    file_name, archive_data, project, site, data_type, start_time, end_time, archive, channels, **kwargs
):
    """
    ...
    try:
        ...
        try:
            ...
            integration_interval = np.tile(
                archive_data["nominal_cadence"], [np.size(channels), np.size(sample_start_time)]
            )
            ...
            data = np.reshape(data[col_idx], [len(col_idx), np.size(sample_start_time)])
...
def _generic_load_converter(
    file_name, archive_data, project, site, data_type, start_time, end_time, archive, channels, **kwargs
):
    return generic_load_converter(
        file_name, archive_data, project, site, data_type, start_time, end_time, archive, channels, **kwargs
    )
...
def generic_save_converter(d, file_name, archive_data):
    ...
    if archive_data["timestamp_method"] == "unixtime":
        ...
        col_offset = 1
        data = np.empty([col_offset + np.size(d.channels), np.size(d.sample_start_time)])
        ...
        get_part = [dt64.get_year, dt64.get_month, dt64.get_day, dt64.get_hour, dt64.get_minute, dt64.get_second]
        for n in range(col_offset):
...
class Data(object):
    ...
    def __init__(
        ...
        self.sample_start_time = np.reshape(sample_start_time, [np.size(sample_start_time)])
        self.sample_end_time = np.reshape(sample_end_time, [np.size(sample_end_time)])
        self.integration_interval = integration_interval
        self.nominal_cadence = nominal_cadence
        if data is None:
            self.data = np.tile(np.nan, [np.size(self.channels), np.size(self.sample_start_time)])
        else:
            self.data = data
    ...
    def __repr__(self):
        units = self.units
        if units == "\N{DEGREE SIGN}C":
            units = "degrees C"
        elif units == "\N{DEGREE SIGN}":
            units = "degrees"
...
"""
    ...
    def __format__(self, fmt):
        ...
        if fmt.startswith("+"):
            ...
            if f in ("start_time", "end_time"):
                ...
                raise ValueError("Invalid format ('%s') for type '%s'" % (fmt, self.__class__.__name__))
        ...
        if conversion is None:
            ...
            raise ValueError("Unknown conversion ('%s') for type '%s'" % (fmt, self.__class__.__name__))
    ...
    def assert_valid(self):
        ...
        try:
            ...
            assert dt64.get_units(self.start_time) == dt64.get_units(
                self.nominal_cadence
            ), "start_time units do not match cadence"
            assert dt64.get_units(self.end_time) == dt64.get_units(
                self.nominal_cadence
            ), "end_time units do not match cadence"
            assert dt64.get_units(self.sample_start_time) == dt64.get_units(
                self.nominal_cadence
            ), "sample_start_time units do not match cadence"
            assert dt64.get_units(self.sample_end_time) == dt64.get_units(
                self.nominal_cadence
    ...
    def extract(
        ...
        channels=None,
        inplace=False,
        straddle_boundary=False,
        trim_straddles=False,
    ):
        ...
        if start_time is not None or end_time is not None:
            ...
            if straddle_boundary:
                tidx = (self.sample_end_time > start_time) & (self.sample_start_time < end_time)
            else:
                tidx = (self.sample_start_time >= start_time) & (self.sample_end_time <= end_time)
            ...
            if straddle_boundary and trim_straddles:
                ...
                r.sample_end_time[r.sample_end_time > end_time] = end_time
    ...
    def sort(self, inplace=False, keep="last"):
        ...
        if self.integration_interval is not None and self.integration_interval.size:
        ...
        **kwargs,
        ...
        if axes is not None:
            ...
            if len(axes2) == 1:
                ...
                assert len(axes2) == len(channels), "axes and channels must be same length"
        elif figure is None:
            figure = plt.figure()
            new_figure = True
        else:
        ...
        if subplot is not None:
            ...
            if len(subplot2) == 1:
                ...
                assert len(subplot2) == len(channels), "subplot and channels must be same length"
        ...
        need_legend = not (
            len(channels) == 1
            or (hasattr(axes, "__iter__") and len(axes) > 1)
            or (hasattr(subplot, "__iter__") and len(subplot) > 1)
        )
        if need_legend:
            ...
            for c in channels:
                ...
                cu = ap.str_units(
                    np.nanmax(np.abs(self.data[allcidx])), self.units, prefix=units_prefix, ascii=False, wantstr=False
                )
        ...
        for n in range(len(channels)):
            ...
            u = ap.str_units(
                np.nanmax(np.abs(self.data[cidx])), self.units, prefix=units_prefix, ascii=False, wantstr=False
            )
            ...
            if "label" in kwargs:
                r.append(dt64.plot_dt64(xdata, ydata, x_time_units=time_units, **kwargs)[0])
            else:
                r.append(dt64.plot_dt64(xdata, ydata, x_time_units=time_units, label=channels[n], **kwargs)[0])
        ...
        if add_legend or (add_legend is None and need_legend):
            ...
            # Label Y axis
            plt.ylabel(str(self.data_description()) + bracket_units(cu["fmtunit"]))

        if new_figure:
            # Add title
            plt.axes(first_axes)
            if title is None:
                plt.title(self.make_title(start_time=start_time, end_time=end_time))
            else:
                plt.title(title)
    ...
    def set_cadence(self, cadence, ignore_nan=True, offset_interval=None, inplace=False, aggregate=None):
        ...
        if cadence > self.nominal_cadence:
            ...
            sample_time = dt64.mean(self.sample_start_time, self.sample_end_time)
            d = np.ones([len(self.channels), len(sam_st)]) * ap.NaN
            if self.integration_interval is not None:
                assert not np.any(dt64.isnat(self.integration_interval)), "integration_interval must not contain NaT"
                integ_intv = np.zeros([len(self.channels), len(sam_st)], dtype=self.integration_interval.dtype)
            else:
                integ_intv = None

            keep_integ_intv = True
            for sn in range(len(sam_st)):
                tidx = np.where(np.logical_and(sample_time >= sam_st[sn], sample_time <= sam_et[sn]))[0]
                for cn in range(len(self.channels)):
                    ...
                    if len(tidx2) != 0:
                        ...
                        if self.integration_interval is None:
                            ...
                            try:
                                d[cn, sn] = aggregate(self.data[cn, tidx2], weights=weights)
            ...
            if inplace:
                ...
                r = copy.copy(self)
                for k in set(self.__dict__.keys()) - set(
                    ["sample_start_time", "sample_end_time", "integration_interval", "nominal_cadence", "data"]
                ):
            ...
            for k in ("start_time", "end_time", "sample_start_time", "sample_end_time", "nominal_cadence"):
    ...
    def mark_missing_data(self, cadence=None, start_time=None, end_time=None):  # inplace=False
        trim = False
        if cadence is None:
        ...
        sample_time = dt64.mean(r.sample_start_time, r.sample_end_time)
    ...
    def interp(self, sample_start_time, sample_end_time, kind="linear"):
        ...
        tu = dt64.smallest_unit(
            [
                dt64.get_units(self.sample_start_time),
        ...
        r.data = scipy.interpolate.interp1d(xi, r.data, kind=kind, bounds_error=False)(xo)
        r.integration_interval = None
        return r

    def space_regularly(self, cadence, start_time=None, end_time=None, missing_cadence=None, kind="linear"):
        if start_time is None:
            start_time = self.start_time
        if end_time is None:
            end_time = self.end_time
        ...
        s = self.mark_missing_data(start_time=start_time, end_time=end_time, cadence=missing_cadence)
    ...
    def least_squares_fit(self, ref, err_func=leastsq_error, inplace=False, full_output=False, plot_fit=False):
        """
        ...
        for c in self.channels:
            ...
            p0 = [0.0]
            fi = scipy.optimize.leastsq(err_func, p0, full_output=True, args=(self, ref, channel))
    ...
    def minimise_sign_error_fit(self, ref, inplace=False, full_output=False, plot_fit=False, **kwargs):
    ...
    def save(self, archive=None, path=None, merge=None, overwrite=True, save_converter=None):
        assert archive is not None or path is not None, "archive or path must be defined"

        an, ai = ap.get_archive_info(self.project, self.site, self.__class__.__name__, archive=archive)
        ...
        if path is None:
            ...
            t = dt64.dt64_range(dt64.floor(self.start_time, duration), dt64.ceil(self.end_time, duration), duration)
        else:
            # Save to single file
            t = [self.start_time]
        ...
        for t1 in t:
            ...
            if merge:
                ...
                tmp = ap.load_data(self.project, self.site, self.__class__.__name__, t1, t2, archive=archive)
                if tmp is not None:
                    if overwrite:
    ...
    def set_time_units(self, units, inplace=False):
        ...
        attr_list = ["start_time", "end_time", "nominal_cadence", "sample_start_time", "sample_end_time"]
        if r.integration_interval is not None:
        ...
        for a in attr_list:
            ...
            if (t1 != t2).any():
                raise ValueError("cannot set units to %s, precision lost in %s", units, a)
            setattr(r, a, t2)
        return r
    ...
    def remove_spikes_chauvenet(
        ...
        self2 = self.space_regularly(self.nominal_cadence, missing_cadence=self.nominal_cadence * 4)
        ...
        for n in range(self2.data.shape[0]):
            self2.data[n] = ap.tools.savitzky_golay(self2.data[n], window_length, 3)
        ...
        outliers = residuals.chauvenet_criterion(window=chauvenet_window, want_outliers=True)
    ...
    def sliding_window(self, func, window, start_time=None, end_time=None):
        ...
        for i in tidx:
            idx2 = np.nonzero(
                np.logical_and(self.sample_start_time >= mst[i] - w2, self.sample_end_time <= mst[i] + w2)
            )[0]
