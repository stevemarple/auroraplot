...
def load_awn_data(file_name, archive_data, project, site, data_type, channels, start_time, end_time, **kwargs):
...
def k_index_filter_battery(mag_data):
    ...
    md_filt = auroraplot.tools.sgolay_filt(mag_data, np.timedelta64(630, "s"), 3)
    md_filt.set_cadence(np.timedelta64(5, "m"), inplace=True)
    return md_filt


def load_bad_data(project, site, data_type, start_time, end_time, archive=None, path=None, extension=".bad", **kwargs):
    ...
    return auroraplot.load_data(project, site, data_type, start_time, end_time, archive=ai[0], path=path, **kwargs)
...
sites = {
    "LAN1": {
        ...
        "data_types": {
            "MagData": {
                ...
                "realtime_baseline": {
                    "channels": np.array(["H", "E", "Z"]),
                    "path": (base_url + "baseline/realtime/lan1/lan1_%Y.txt"),
    ...
    "EXE": {
        ...
        "copyright": "\xa9 Crown Copyright, Met Office",
    ...
    "SUM": {
        ...
        "line_color": [0, 0x65 / 255.0, 0xCC / 255.0],  # Blue from Shetland flag
    },  # SUM
}
...
default_data_types = {
    ...
    "TemperatureData": {
        "realtime": {
            "channels": np.array(["Sensor temperature", "System temperature"]),
            "path": base_url + "{site_lc}/%Y/%m/{site_lc}_%Y%m%d.txt",
            "duration": np.timedelta64(24, "h"),
            "format": "aurorawatchnet",
            "load_converter": load_awn_data,
        ...
        for an, av in default_data_types[dt].items():
            if an not in sdt[dt]:
                ...
                if not hasattr(sdt[dt][an]["path"], "__call__"):
                    sdt[dt][an]["path"] = sdt[dt][an]["path"].format(site_lc=site_lc)
            elif sdt[dt] is None:
...
for s in ("LAN1", "EXE", "SID", "SUM", "TEST1"):
    ...
    sites[s]["data_types"]["MagData"]["realtime_baseline"]["fmt"] = ["%04d", "%02d", "%02d", "%.2f", "%.2f", "%.2f"]
