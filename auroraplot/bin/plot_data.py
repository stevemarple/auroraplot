...
# from auroraplot.bin.plot_mag_data import agg_func
...
# Import all datasets by wildcard so this plot program will be aware when new datasets are added.
...
def parse_args() -> argparse.Namespace:
    ...
    plot_type.add_argument(
        "--riometer-raw-power-plot",
        dest="plot_type",
        action="store_const",
        const="rio_rawpower_plot",
        help="Make riometer raw power plot(s)",
    )
    plot_type.add_argument(
        "--riometer-power-plot",
        dest="plot_type",
        action="store_const",
        const="rio_power_plot",
        help="Make riometer power plot(s)",
    )
    plot_type.add_argument(
...
def main():
    ...
    # Requesting the UIT or DTU data from the UIT website requires a
    # password to be set. Try reading the password from a file called
    # .uit_password from the user's home directory.
    if ("UIT" in project_list or "DTU" in project_list) and auroraplot.datasets.uit.uit_password is None:
    ...
    qdc_archive = ap.parse_archive_selection([])
    baseline_archive = ap.parse_archive_selection(default_baseline_archive_selection)

    # Process --archive options
    if args.archive:
        archive = ap.parse_archive_selection(args.archive, defaults=archive)
    if args.qdc_archive:
        qdc_archive = ap.parse_archive_selection(args.qdc_archive, defaults=qdc_archive)
    ...
    elif args.plot_type == "rio_rawpower_plot":
        data_type = "RioRawPower"
    elif args.plot_type == "rio_power_plot":
        data_type = "RioPower"
    else:
        data_type = "MagData"
    ...
    if args.plot_type is None or args.plot_type == "stack_plot":
        if len(mdl) == 1:
            ...
            if args.with_qdc:
                if mdl[0].project in qdc_archive and mdl[0].site in qdc_archive[mdl[0].project]:
    ...
    if args.save_filename and fig is not None:
