...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("-n", "--dry-run", action="store_true", help="Dry run, copy but do not save")
    parser.add_argument(
    ...
    parser.add_argument("--raise-all", action="store_true", help="Raise all exceptions, do not continue on error")
    parser.add_argument(
        "--log-level",
...
def main():
    ...
    for n in range(len(project_list)):
        ...
        dest_an, dest_ai = ap.get_archive_info(project, site, "MagData", archive=archive)

        src_an, src_ai = ap.get_archive_info(project, site, "MagData", archive="original_" + dest_an)
            ...
            try:
                if hasattr(dest_path, "__call__"):
                    ...
                    dest_file_name = dest_path(
                        t, project=project, site=site, data_type=data_type, archive=dest_an, channels=channels
                    )
                else:
                ...
                if hasattr(src_path, "__call__"):
                    ...
                    # the src_path
                    file_name = src_path(
                        t, project=project, site=site, data_type=data_type, archive=src_an, channels=channels
                    )
                else:
                ...
                if os.path.exists(dest_file_name) and os.path.samefile(file_name, dest_file_name):
