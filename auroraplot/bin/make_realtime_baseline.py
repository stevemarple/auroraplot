...
from auroraplot.magdata import load_qdc as load_mag_qdc
...
def main():
    ...
    project_list, site_list = auroraplot.parse_project_site_list(args.project_site)

    # Process --archive options for source data
    if args.archive:
        archive = auroraplot.parse_archive_selection(args.archive)
    else:
        archive = {}

    # Process --baseline-archive options for target data
    if args.baseline_archive:
        baseline_archive = auroraplot.parse_archive_selection(args.baseline_archive)
    else:
        baseline_archive = {}
    ...
    for pn in range(len(project_list)):
        project = project_list[pn]
        site = site_list[pn]
