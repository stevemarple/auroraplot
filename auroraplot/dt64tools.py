...
log10_to_unit = {-18: "as", -15: "fs", -12: "ps", -9: "ns", -6: "us", -3: "ms", 0: "s"}
...
def from_ymd(year, month, day):
    ...
    for a in [ya, ma, da]:
        if a.size != 1:
            assert r_shape is None or r_shape == a.shape, "Shapes must match or be scalar"
            r_shape = a.shape
    if r_shape is None:
        r_shape = [1]
...
def _aggregate(*a, **kwargs):
    ...
    if len(a) == 1:
        ...
        for b in a[1:]:
            ...
            assert isinstance(b.dtype.type(), a0.dtype.type), "Arrays must hold the same data type"
...
def t_to_axis_value(t, axis, units=None, epoch=None, fmt=None):
    ...
    if hasattr(axis, "dt64tools"):
        ...
        assert time_units.index(t_units) >= time_units.index(axis_data.units), (
            "Cannot add time data with units " + t_units + " to existing plot using units " + axis_data.units
        )
        ...
        axis_data = Dt64ToolsData(units=units, type=t.dtype.type, epoch=dt64_to(epoch, units).astype("int64"))
        axis.dt64tools = axis_data
...
def highlight(ax, st, et, color="y", **kwargs):
    ...
    x1 = dt64_to(st, u)
    x2 = dt64_to(et, u)
    ax.add_patch(patches.Rectangle([x1, yl[0]], x2 - x1, yl[1] - yl[0], **kwargs))
...
def get_sidereal_day(units: Union[str, np.datetime64, np.timedelta64] = "s") -> numpy.timedelta64:
    ...
    if isinstance(units, (np.datetime64, np.timedelta64)):
        units = get_units(units)
    sd = np.timedelta64(23, "h") + np.timedelta64(56, "m") + np.timedelta64(4, "s") + np.timedelta64(100, "ms")
...
def lmst_to_utc(
    lmst: Union[np.timedelta64, np.ndarray],
    longitude_deg: Union[float, np.ndarray],
    d: Union[np.datetime64, np.ndarray],
...
def get_sidereal_time_offset(
    t: Union[np.datetime64, np.ndarray],
    longitude_deg: float,
    st: Optional[Union[np.datetime64, np.ndarray]] = None,
    units: str = "ms",
) -> Union[Union[np.timedelta64, np.ndarray], Tuple[Union[np.timedelta64, np.ndarray], Union[int, np.ndarray]]]:
...
class Datetime64Locator(Locator):
    ...
    def __call__(self):
        ...
        limits_dt64 = (epoch + limits).astype("m8[" + units + "]") + self.axis.dt64tools.type(0, units)
        ...
        if tick_interval is not None:
            ...
            num = np.floor(((last_tick - first_tick) / tick_interval) + 1).astype("int64")
            tick_locs = (
                (dt64_to(tick_interval, units).astype("int64") * np.arange(num)) + first_tick.astype("int64")
            ) - epoch
            ...
            return self.raise_if_exceeds(self.calc_irregular_ticks(limits_dt64[0], limits_dt64[1], units))
    ...
    def calc_interval(self, st, et):
        ...
        if approx_ival_s < 1.0:
            ...
            intervals = (np.array([1, 2, 4, 5]) * 10 ** int(np.floor(approx_ival_log10 - unit_log10))).astype(
                "m8[" + best_unit + "]"
            )
            approx_ival = np.timedelta64(int(approx_ival_s * 10**-unit_log10), best_unit)
            try:
...
class Datetime64Formatter(Formatter):
    ...
    def __call__(self, x, pos=None):
        ...
        if pos is None:
            ...
            if len(tick_locs) >= 2:
                tick_interval = np.timedelta64(int((tick_locs[-1] - tick_locs[0]) / (len(tick_locs) - 1)), units)
            else:
                tick_interval = np.timedelta64(int(np.diff(self.axis.get_view_interval())), units)
            f = []
            ...
            if data_interval > np.timedelta64(1, "D"):
                # f.append('%Y-%m-%d')
                if tick_interval is not None and tick_interval >= np.timedelta64(365, "D"):
                    f.append("%Y")
                else:
                    f.append("%Y-%m-%d")
        ...
        if pos == 1:
            ...
            if self.autolabel or (self.autolabel is None and self.axis.get_label().get_text() == ""):
