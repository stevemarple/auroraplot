...
def load_iaga_2000(file_name, archive_data, project, site, data_type, channels, start_time, end_time, **kwargs):
    ...
    for c in channels:
        ...
        if "uit_channels" in archive_data:
            ...
            for poss_name in [site.upper() + c.upper(), "---" + c.upper()]:
        ...
        data.append([float("nan") if x == "99999.00" else float(x) for x in iaga["data"][n]])
...
def uit_path(t, project, site, data_type, archive, channels):
    ...
    # including password.
    a, d = copy.deepcopy(ap.get_archive_info(project, site, data_type, archive=archive))
