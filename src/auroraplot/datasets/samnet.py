...
def load_samnet_data(file_name, archive_data, project, site, data_type, channels, start_time, end_time, **kwargs):
    chan_tup = tuple(archive_data["channels"])
    col_idx = []
    for c in channels:
        col_idx.append(chan_tup.index(c))
    nominal_cadence_s = archive_data["nominal_cadence"] / np.timedelta64(1000000, "us")
    try:
        try:
            conv = lambda s: (s.strip().startswith("9999.9") and np.nan) or float(s.strip())
            comments = ap.get_site_info(project, site, "samnet_code")[0]
            data = np.loadtxt(file_name, unpack=True, converters={0: conv, 1: conv, 2: conv}, comments=comments)
...
def load_new_samnet_data(file_name, archive_data, project, site, data_type, channels, start_time, end_time, **kwargs):
    ...
    try:
        if os.path.exists(file_name):
            ...
            fh = req.raw
        try:
            file_data = np.loadtxt(fh, unpack=True, comments="%", converters={3: to_tesla, 4: to_tesla, 5: to_tesla})
...
def load_new_samnet_temp_volt_data(
    file_name, archive_data, project, site, data_type, channels, start_time, end_time, **kwargs
):
    ...
    try:
        ...
        try:
            file_data = np.loadtxt(
                ...
                unpack=True,
                comments="%",
                converters={2: to_float, 3: to_float, 4: to_float, 5: to_float, 6: to_float, 7: to_cpu_temp},
            )
            assert np.all(file_data[0] >= 0) and np.all(file_data[0] < 24)
            assert np.all(file_data[1] >= 0) and np.all(file_data[1] < 60)
            ...
            sample_start_time = (
                start_time + file_data[0].astype("m8[h]") + file_data[1].astype("m8[m]") + np.timedelta64(0, "us")
            )
...
def load_rt_data(file_name, archive_data, project, site, data_type, channels, start_time, end_time, **kwargs):
...
sites = {
    "BOR1": {
        ...
        "elevation": np.nan,
        "start_time": np.datetime64("1998-12-03T00:00:00", "s"),
        "end_time": np.datetime64("2015-06-29T00:00:00", "s"),
    ...
    "CRK1": {
        ...
        "elevation": np.nan,
        "start_time": np.datetime64("2002-05-17T00:00:00", "s"),
        "end_time": np.datetime64("2013-02-26T00:00:00", "s"),
    ...
    "CRK2": {
        ...
        "longitude": Decimal("-2.64"),
        "elevation": np.nan,
        "start_time": np.datetime64("2015-08-18T00:00:00", "s"),
        "end_time": None,
        "k_index_scale": 800e-9,  # Estimated
        "copyright": "Lancaster University",
        "license": "Data users are not entitled to distribute data to third parties outside their own research teams without requesting permission from Prof. F. Honary. Similarly, SAMNET data should not become part of a distributed database without permission first being sought. Commercial use prohibited.",
        ...
        "data_types": {
            ...
            "TemperatureData": {
                "1min": {
                    "channels": np.array(
                        ["Sensor temperature", "System temperature", "Obsdaq temperature", "CPU temperature"]
                    ),
    ...
    "ESK1": {
        ...
        "elevation": 245,
        "start_time": np.datetime64("2001-01-01T00:00:00", "s"),
        "end_time": None,
    ...
    "FAR1": {
        ...
        "elevation": np.nan,
        "start_time": np.datetime64("1987-10-01T00:00:00", "s"),
        "end_time": np.datetime64("2011-02-12T00:00:00", "s"),
        "copyright": "Lancaster University",
        "license": "Data users are not entitled to distribute data to third parties outside their own research teams without requesting permission from Prof. F. Honary. Similarly, SAMNET data should not become part of a distributed database without permission first being sought. Commercial use prohibited.",
        "attribution": "The Sub-Auroral Magnetometer Network (SAMNET) is operated by the Space and Plasma Physics group, Department of Physics, Lancaster University.",
        "samnet_code": "fa",
    ...
    "GML1": {
        ...
        "elevation": np.nan,
        "start_time": np.datetime64("1987-10-01T00:00:00", "s"),
        "end_time": np.datetime64("2002-05-15T00:00:00", "s"),
        "copyright": "Lancaster University",
        "license": "Data users are not entitled to distribute data to third parties outside their own research teams without requesting permission from Prof. F. Honary. Similarly, SAMNET data should not become part of a distributed database without permission first being sought. Commercial use prohibited.",
        "attribution": "The Sub-Auroral Magnetometer Network (SAMNET) is operated by the Space and Plasma Physics group, Department of Physics, Lancaster University.",
        "samnet_code": "gm",
    ...
    "HAN1": {
        ...
        "elevation": np.nan,
        "start_time": np.datetime64("1997-01-03T00:00:00", "s"),
        "end_time": np.datetime64("2005-05-16T00:00:00", "s"),
    ...
    "HAN3": {
        ...
        "elevation": np.nan,
        "start_time": np.datetime64("2005-08-18T00:00:00", "s"),
        "end_time": None,
        "copyright": "Finnish Meteorological Institute / Lancaster University",
        "license": "Data users are not entitled to distribute data to third parties outside their own research teams without requesting permission from Prof. F. Honary. Similarly, SAMNET data should not become part of a distributed database without permission first being sought. Commercial use prohibited.",
        "attribution": "The Sub-Auroral Magnetometer Network (SAMNET) is operated by the Space and Plasma Physics group, Department of Physics, Lancaster University. Original data provided by the Finnish Meteorological Institute.",
    ...
    "HLL1": {
        ...
        "elevation": np.nan,
        "start_time": np.datetime64("1998-10-06T00:00:00", "s"),
        "end_time": np.datetime64("2011-05-19T00:00:00", "s"),
        "copyright": "Lancaster University",
        "license": "Data users are not entitled to distribute data to third parties outside their own research teams without requesting permission from Prof. F. Honary. Similarly, SAMNET data should not become part of a distributed database without permission first being sought. Commercial use prohibited.",
        "attribution": "The Sub-Auroral Magnetometer Network (SAMNET) is operated by the Space and Plasma Physics group, Department of Physics, Lancaster University.",
        "samnet_code": "hl",
    ...
    "KIL1": {
        ...
        "elevation": np.nan,
        "start_time": np.datetime64("1997-12-01T00:00:00", "s"),
        "end_time": None,
        "copyright": "Finnish Meteorological Institute / Lancaster University",
        "license": "Data users are not entitled to distribute data to third parties outside their own research teams without requesting permission from Prof. F. Honary. Similarly, SAMNET data should not become part of a distributed database without permission first being sought. Commercial use prohibited.",
        "attribution": "The Sub-Auroral Magnetometer Network (SAMNET) is operated by the Space and Plasma Physics group, Department of Physics, Lancaster University. Original data provided by the Finnish Meteorological Institute.",
    ...
    "KVI1": {
        ...
        "elevation": np.nan,
        "start_time": np.datetime64("1987-10-13T00:00:00", "s"),
        "end_time": np.datetime64("2003-01-01T00:00:00", "s"),
        "copyright": "Lancaster University",
        "license": "Data users are not entitled to distribute data to third parties outside their own research teams without requesting permission from Prof. F. Honary. Similarly, SAMNET data should not become part of a distributed database without permission first being sought. Commercial use prohibited.",
        "attribution": "The Sub-Auroral Magnetometer Network (SAMNET) is operated by the Space and Plasma Physics group, Department of Physics, Lancaster University.",
        "samnet_code": "kv",
    ...
    "LAN1": {
        ...
        "elevation": np.nan,
        "start_time": np.datetime64("2003-02-20T17:00:00", "s"),
        "end_time": np.datetime64("2013-10-24T00:00:00", "s"),
    ...
    "LAN2": {
        ...
        "longitude": Decimal("-2.77"),
        "elevation": np.nan,
        "start_time": np.datetime64("2016-03-21T18:31:00", "s"),
        "end_time": None,
        "k_index_scale": 650e-9,  # Estimated
        "copyright": "Lancaster University",
        "license": "Data users are not entitled to distribute data to third parties outside their own research teams without requesting permission from Prof. F. Honary. Similarly, SAMNET data should not become part of a distributed database without permission first being sought. Commercial use prohibited.",
        "attribution": "The Sub-Auroral Magnetometer Network (SAMNET) is operated by the Space and Plasma Physics group, Department of Physics, Lancaster University.",
        "line_color": [181 / 255.0, 18 / 255.0, 27 / 255.0],
        "samnet_code": "la",
        "data_types": {
            "MagData": {
                "default": "1s",
    ...
    "LNC2": {
        ...
        "longitude": Decimal("-2.77"),
        "elevation": np.nan,
        "start_time": np.datetime64("2006-12-24T00:00:00", "s"),
        "end_time": np.datetime64("2008-06-27T00:00:00", "s"),
        "copyright": "Lancaster University",
        "license": "Data users are not entitled to distribute data to third parties outside their own research teams without requesting permission from Prof. F. Honary. Similarly, SAMNET data should not become part of a distributed database without permission first being sought. Commercial use prohibited.",
        "attribution": "The Sub-Auroral Magnetometer Network (SAMNET) is operated by the Space and Plasma Physics group, Department of Physics, Lancaster University.",
        "samnet_code": "ln",
    ...
    "NOR1": {
        ...
        "elevation": np.nan,
        "start_time": np.datetime64("1987-10-01T00:00:00", "s"),
        "end_time": np.datetime64("2003-03-01T00:00:00", "s"),
        "copyright": "Lancaster University",
        "license": "Data users are not entitled to distribute data to third parties outside their own research teams without requesting permission from Prof. F. Honary. Similarly, SAMNET data should not become part of a distributed database without permission first being sought. Commercial use prohibited.",
        "attribution": "The Sub-Auroral Magnetometer Network (SAMNET) is operated by the Space and Plasma Physics group, Department of Physics, Lancaster University.",
        "samnet_code": "no",
    ...
    "NUR3": {
        ...
        "elevation": 105,
        "start_time": np.datetime64("2003-01-01T00:00:00", "s"),
        "end_time": None,
        "copyright": "Lancaster University",
        "license": "Data users are not entitled to distribute data to third parties outside their own research teams without requesting permission from Prof. F. Honary. Similarly, SAMNET data should not become part of a distributed database without permission first being sought. Commercial use prohibited.",
        "attribution": "The Sub-Auroral Magnetometer Network (SAMNET) is operated by the Space and Plasma Physics group, Department of Physics, Lancaster University.",
    ...
    "YOR1": {
        ...
        "elevation": np.nan,
        "start_time": np.datetime64("1987-10-01T00:00:00", "s"),
        "end_time": np.datetime64("2006-03-30T12:00:00", "s"),
        "copyright": "Lancaster University",
        "license": "Data users are not entitled to distribute data to third parties outside their own research teams without requesting permission from Prof. F. Honary. Similarly, SAMNET data should not become part of a distributed database without permission first being sought. Commercial use prohibited.",
        "attribution": "The Sub-Auroral Magnetometer Network (SAMNET) is operated by the Space and Plasma Physics group, Department of Physics, Lancaster University.",
        "samnet_code": "yo",
...
default_activity_colors = np.array(
    [[0.2, 1.0, 0.2], [1.0, 1.0, 0.0], [1.0, 0.6, 0.0], [1.0, 0.0, 0.0]]  # green  # yellow  # amber
)  # red
...
default_data_types = {
    "MagData": {
        ...
        "realtime": {
            "channels": sam_channels,
            "path": (base_url + "realtime/{site_lc}/%Y/%m/{site_lc}%Y%m%d.rt"),
            "duration": np.timedelta64(24, "h"),
        ...
        "realtime_baseline": {
            "channels": sam_channels,
            "path": (base_url + "baseline/realtime/{site_lc}/{site_lc}_%Y.txt"),
        ...
        for an, av in default_data_types[dt].items():
            if an not in sdt[dt]:
                ...
                if not hasattr(sdt[dt][an]["path"], "__call__"):
                    sdt[dt][an]["path"] = sdt[dt][an]["path"].format(site_lc=site_lc, sc=sc)
            elif sdt[dt] is None:
