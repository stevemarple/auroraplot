...
def load_bgs_sch_data(
    file_name, archive_data, project, site, data_type, channels, start_time, end_time, invalid_raise=False, **kwargs
):
    ...
    col_idx = []
    for c in channels:
        col_idx.append(data_type_info[data_type]["col_offset"] + chan_tup.index(c))
    try:
        ...
        try:
            ...
            data = np.genfromtxt(uh, unpack=True, invalid_raise=invalid_raise, **kw)
...
def remove_spikes(md, **kwargs):
    return md.remove_spikes_chauvenet(
        savgol_window=np.timedelta64(5, "m"), chauvenet_window=np.array([89, 79]).astype("timedelta64[s]")
    )
...
def temperature_compensation(md, inplace=False, cadence=None, **kwargs):
    ...
    if cadence and cadence > cad:
        cad = cadence
    td = ap.load_data(
        md.project, md.site, "TemperatureData", md.start_time, md.end_time, channels=["Sensor temperature"], cadence=cad
    )
    # Resample
...
sites = {
    "BHM1": {  # Previously BGS3 at Lancaster
        ...
        "line_color": [86 / 255.0, 146 / 255.0, 206 / 255.0],
        "data_types": {
            "MagData": {
                "default": "realtime",
                "raw": {
                ...
                "realtime_baseline": {
                    "channels": np.array(["H", "E", "Z"]),
                    "path": (data_dir + "/baseline/realtime/bgs3/bgs3_%Y.txt"),
    ...
    "BHM2": {  # Formerly LAN1 and BGS4
        ...
        "data_types": {
            "MagData": {
                ...
                "realtime_baseline": {
                    "channels": np.array(["H", "E", "Z"]),
                    "path": (data_dir + "/baseline/realtime/bgs4/bgs4_%Y.txt"),
                    "duration": np.timedelta64(1, "Y"),
                    "load_converter": ap.data.generic_load_converter,
                    "save_converter": ap.data.generic_save_converter,
                    "nominal_cadence": np.timedelta64(1, "D"),
    ...
    "LAN2": {  # Formerly BGS5
        ...
        "line_color": [0x7B / 255.0, 0x03 / 255.0, 0x48 / 255.0],
        "data_types": {
            "MagData": {
                "default": "realtime",
                "raw": {
                ...
                "realtime_baseline": {
                    "channels": np.array(["H", "E", "Z"]),
                    "path": (data_dir + "/baseline/realtime/lan2/lan2_%Y.txt"),
                    "duration": np.timedelta64(1, "Y"),
                    "load_converter": ap.data.generic_load_converter,
                    "save_converter": ap.data.generic_save_converter,
                    "nominal_cadence": np.timedelta64(1, "D"),
    ...
    "ALE": {  # Previously BGS7
        ...
        "line_color": [0x60 / 255.0, 0x00 / 255.0, 0x00 / 255.0],
        ...
        "data_types": {
            "MagData": {
                ...
                "realtime_baseline": {
                    "channels": np.array(["H", "E", "Z"]),
                    "path": (data_dir + "/baseline/realtime/bgs7/ale_%Y.txt"),
                    "duration": np.timedelta64(1, "Y"),
                    "load_converter": ap.data.generic_load_converter,
                    "save_converter": ap.data.generic_save_converter,
                    "nominal_cadence": np.timedelta64(1, "D"),
    ...
    "BEN": {  # Previously BGS6
        ...
        "license": cc3_by_nc_sa,
        "attribution": "British Geological Survey.",
        "line_color": [0xDA / 255.0, 0x25 / 255.0, 0x1D / 255.0],
        ...
        "data_types": {
            "MagData": {
                ...
                "realtime_baseline": {
                    "channels": np.array(["H", "E", "Z"]),
                    "path": (data_dir + "/baseline/realtime/ben/ben_%Y.txt"),
                    "duration": np.timedelta64(1, "Y"),
                    "load_converter": ap.data.generic_load_converter,
                    "save_converter": ap.data.generic_save_converter,
                    "nominal_cadence": np.timedelta64(1, "D"),
    ...
    "BRO": {  # Formerly BGS8
        ...
        "license": cc3_by_nc_sa,
        "attribution": "British Geological Survey.",
        "line_color": [0x07 / 255.0, 0x0E / 255.0, 0x68 / 255.0],
        ...
        "data_types": {
            "MagData": {
                ...
                "realtime_baseline": {
                    "channels": np.array(["H", "E", "Z"]),
                    "path": (data_dir + "/baseline/realtime/bro/bro_%Y.txt"),
                    "duration": np.timedelta64(1, "Y"),
                    "load_converter": ap.data.generic_load_converter,
                    "save_converter": ap.data.generic_save_converter,
                    "nominal_cadence": np.timedelta64(1, "D"),
    ...
    "NOR": {  # Previously BGS9
        ...
        "license": cc3_by_nc_sa,
        "attribution": "British Geological Survey.",
        "line_color": [0x70 / 255.0, 0x00 / 255.0, 0x3B / 255.0],
        ...
        "data_types": {
            "MagData": {
                ...
                "realtime_baseline": {
                    "channels": np.array(["H", "E", "Z"]),
                    "path": (data_dir + "/baseline/realtime/nor/nor_%Y.txt"),
                    "duration": np.timedelta64(1, "Y"),
                    "load_converter": ap.data.generic_load_converter,
                    "save_converter": ap.data.generic_save_converter,
                    "nominal_cadence": np.timedelta64(1, "D"),
    ...
    "OUN": {  # Previously BGS10
        ...
        "license": cc3_by_nc_sa,
        "attribution": "British Geological Survey.",
        "line_color": [0x05 / 255.0, 0x31 / 255.0, 0x61 / 255.0],
        ...
        "data_types": {
            "MagData": {
                ...
                "realtime_baseline": {
                    "channels": np.array(["H", "E", "Z"]),
                    "path": (data_dir + "/baseline/realtime/oun/oun_%Y.txt"),
                    "duration": np.timedelta64(1, "Y"),
                    "load_converter": ap.data.generic_load_converter,
                    "save_converter": ap.data.generic_save_converter,
                    "nominal_cadence": np.timedelta64(1, "D"),
    ...
    "DUN": {  # Previously BGS2
        ...
        "license": cc3_by_nc_sa,
        "attribution": "British Geological Survey.",
        "line_color": [0x11 / 255.0, 0x41 / 255.0, 0x8C / 255.0],
        ...
        "data_types": {
            "MagData": {
                ...
                "realtime_baseline": {
                    "channels": np.array(["H", "E", "Z"]),
                    "path": (data_dir + "/baseline/realtime/dun/oun_%Y.txt"),
                    "duration": np.timedelta64(1, "Y"),
                    "load_converter": ap.data.generic_load_converter,
                    "save_converter": ap.data.generic_save_converter,
                    "nominal_cadence": np.timedelta64(1, "D"),
    ...
    "ESB": {  # Previously BGS1
        ...
        "license": cc3_by_nc_sa,
        "attribution": "British Geological Survey.",
        "line_color": [0x73 / 255.0, 0x00 / 255.0, 0x28 / 255.0],
        ...
        "data_types": {
            "MagData": {
                ...
                "realtime_baseline": {
                    "channels": np.array(["H", "E", "Z"]),
                    "path": (data_dir + "/baseline/realtime/esb/oun_%Y.txt"),
                    "duration": np.timedelta64(1, "Y"),
                    "load_converter": ap.data.generic_load_converter,
                    "save_converter": ap.data.generic_save_converter,
                    "nominal_cadence": np.timedelta64(1, "D"),
    ...
    for dt in default_data_types:
        if dt not in sd["data_types"]:
            ...
            for archive in sd["data_types"][dt]:
                if archive == "default":
                    continue
                sd["data_types"][dt][archive]["path"] = sd["data_types"][dt][archive]["path"].format(site_lc=s.lower())
