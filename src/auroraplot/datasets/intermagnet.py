...
def load_iaga_2002(file_name, archive_data, project, site, data_type, channels, start_time, end_time, **kwargs):
...
for sa in sites:
    ...
    if "preliminary" not in s["data_types"]["MagData"]:
        s["data_types"]["MagData"]["preliminary"] = {
            ...
            "path": "ftp://" + ftp_hostname + "/preliminary/%Y/%m/IAGA2002/" + sa.lower() + "%Y%m%dvmin.min",
    ...
    if "definitive_minute" not in s["data_types"]["MagData"]:
        s["data_types"]["MagData"]["definitive_minute"] = {
            "channels": ["X", "Y", "Z", "F"],
            "path": "ftp://" + ftp_hostname + "/minute/definitive/IAGA2002/%Y/%m/" + sa.lower() + "%Y%m%ddmin.min.gz",
            "duration": np.timedelta64(24, "h"),
            "load_converter": load_iaga_2002,
