...
class AuroraWatchActivity(Data):
    ...
    def __init__(
        ...
        if magdata is not None and magqdc is not None:
            ...
            nth_largest = ap.tools.NthLargest(n)
            self.set_cadence(np.timedelta64(60, "m"), inplace=True, aggregate=nth_largest)
    ...
    def get_color(self):
        ...
        assert np.all(self.thresholds == np.sort(self.thresholds)), "thresholds not in ascending order"
...
class KIndex(Data):
    ...
    def __init__(
        ...
        if magdata is not None:
            ...
            self.start_time = dt64.floor(magdata.start_time, self.nominal_cadence)
            self.end_time = dt64.ceil(magdata.end_time, self.nominal_cadence)

            self.sample_start_time = np.arange(self.start_time, self.end_time, self.nominal_cadence)
            ...
            if magqdc is None:
                ...
                bsub = np.abs(magdata.data[magdata.get_channel_index(c)] - aligned.data[aligned.get_channel_index(c)])
                self.with_qdc = False
            ...
            self.range = np.zeros([len(self.channels), len(self.sample_start_time)])
            for i in range(len(self.sample_start_time)):
                tidx = np.where(
                    np.logical_and(
                        magdata.sample_start_time >= self.sample_start_time[i],
                        magdata.sample_end_time <= self.sample_end_time[i],
                    )
                )[0]
                for cn in range(len(self.channels)):
                    ...
                    self.range[cn, i] = nth_largest(bsub[cn, tidx]) - nth_smallest(bsub[cn, tidx])
            ...
            self.thresholds = np.array([0.00, 0.01, 0.02, 0.04, 0.08, 0.14, 0.24, 0.40, 0.66, 1.00]) * scale
        ...
        **kwargs,
        ...
        if plot_func == plt.bar:
            ...
            if time_units is None:
                time_units = dt64.get_units(self.sample_start_time)
            if "width" not in kwargs:
                kwargs["width"] = dt64.dt64_to(self.nominal_cadence, time_units) * 1.0
